import React, { useState } from 'react';
import { 
    View,
    Text,
    StyleSheet,
    Image,
    ScrollView,
} from 'react-native';
import { heightPercentageToDP as hp, widthPercentageToDP as wp } from 'react-native-responsive-screen';
import Icon from 'react-native-vector-icons/Ionicons';
import { black, darkGray, gray, primary, primaryPurple, white } from '../../../assets/colors';
import { bold, medium, regular } from '../../../assets/fonts';
import ServiceCard from '../../../components/cards/ServiceCard';
import { Container } from '../../../components/common';
import DonationCard from '../../../components/home/DonationCard';
import { digitalServices, localServices } from '../../../utils/RawData';

const Home = ({navigation}) => {

    return(
        <Container style={{padding: hp('2.5%')}}>
            <ScrollView showsVerticalScrollIndicator={false}>
                <View style={styles.headerRow}>
                    <Image source={require('../../../assets/images/person.jpg')} style={styles.userImage}/>
                    <Icon name="search-outline" size={hp('5%')} color={primary}/>
                </View>
                <Text style={styles.name}>Hi, <Text style={[styles.name,{color: primary}]}>Jason!</Text></Text>
                <Text style={styles.text}>How are you? Here are a few causes you would love donating to.</Text>
                <Text style={[{borderTopWidth:0.75, borderColor: gray, fontSize: hp('2.75%'), paddingTop: hp('2%'), color: black, fontFamily: bold}]}>Most Recent</Text>


                <DonationCard
                    image={require('../../../assets/images/ngo1.png')}
                    text={"Yomiruh Widows"}
                    onPress={null}
                />
                <DonationCard
                    image={require('../../../assets/images/ngo2.png')}
                    text={"Yomiruh Widows"}
                    onPress={null}
                />
                <DonationCard
                    image={require('../../../assets/images/ngo3.png')}
                    text={"Yomiruh Widows"}
                    onPress={null}
                />
                <DonationCard
                    image={require('../../../assets/images/ngo4.png')}
                    text={"Yomiruh Widows"}
                    onPress={null}
                />
                <DonationCard
                    image={require('../../../assets/images/ngo5.png')}
                    text={"Yomiruh Widows"}
                    onPress={null}
                />
            </ScrollView>
        </Container>
    )
}

export default Home;

const styles = StyleSheet.create({
    headerRow:{
        flexDirection: 'row',
        justifyContent: 'space-between',
        alignItems: 'center',
        marginVertical: hp('1%')
    },
    userImage:{
        height: hp('8%'),
        width: hp('8%'),
        borderRadius: 100,
    },
    name: {
        fontFamily: bold,
        color: black,
        fontSize: hp('3.5%'),
        marginVertical: hp('1%'),
    },
    text:{
        fontFamily: regular,
        color: darkGray,
        fontSize: hp('2.25%'),
        width: wp('70%'),
        paddingVertical: hp('1%'),
        paddingBottom: hp('2%'),
    }
})